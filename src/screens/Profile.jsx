...
// ИМПОРТИРУЕМ НАШ ХУК ЯЗЫКА
import { useLanguage } from '../i18n/LanguageContext'; 

const Profile = () => {
  const { lang, setLang, t } = useLanguage(); // Достаем язык и функцию перевода t()
  
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [tgUser, setTgUser] = useState(null);
  const [copyStatus, setCopyStatus] = useState(t('invite_btn')); // Используем перевод
  ...
  useEffect(() => {
    ...
    setCopyStatus(t('invite_btn')); // Обновляем кнопку при смене языка
  }, [lang]); // Зависимость от lang
    ...
    <div className="min-h-full px-5 pt-4 pb-32 animate-in fade-in duration-500">
      ...
      <div className="w-full rounded-[32px] bg-gradient-to-b from-[#3b1d52] via-[#241535] to-[#161618] border border-white/5 p-6 mb-8 shadow-2xl">
        <div className="flex flex-col items-center">
          <div className="w-auto h-20 px-6 py-2 rounded-full flex items-center gap-4 bg-white/5 border-2 border-white/10 mb-6 shadow-xl">
              <div className="w-16 h-16 rounded-full overflow-hidden border border-white/10 bg-black shrink-0">
                <img src={tgUser.avatar} alt="avatar" className="w-full h-full object-cover" />
              </div>
              <h2 className="text-2xl font-bold tracking-tight pr-2">{tgUser.username}</h2>
          </div>
          ...
          <button className="w-auto py-2 px-6 rounded-full flex items-center justify-center gap-1 text-zinc-400 text-sm font-medium bg-white/5">
      ...
      <div className="w-full rounded-[32px] bg-gradient-to-br from-[#4a246b] via-[#2c1345] to-[#18181b] p-6 mb-6 shadow-2xl">
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-2">
            <span className="text-lg font-bold">{t('level')} 1</span>
            <span className="bg-white/20 text-white text-[11px] font-black px-2 py-1 rounded-full">5%</span>
          </div>
        </div>

        <div className="flex gap-3">
          <div className="flex-1 bg-white/5 rounded-full px-6 py-5 flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
              <WalletCards size={16} className="text-purple-400"/><span className="text-sm font-medium">{t('earned')}</span>
            </div>
            <div className="text-2xl font-bold flex items-center gap-1">0 <span className="text-zinc-500 text-lg">💎</span></div>
          </div>
          <div className="flex-1 bg-white/5 rounded-full px-6 py-5 flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
      ...
      {/* КНОПКА ПРИГЛАШЕНИЯ */}
      <button onClick={handleInvite} className="w-full h-16 bg-[#a855f7] rounded-full font-bold text-lg text-white flex items-center justify-center gap-3 active:scale-95 shadow-[0_0_30px_rgba(168,85,247,0.3)] pr-4">
